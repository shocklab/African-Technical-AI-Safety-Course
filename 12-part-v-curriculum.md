...
## ⚠ Correction flagged for existing (already-audited) pages
Three pages call RIA's **Africa Just AI** a "framework"; it is a research project. To fix when convenient: the **8.3 page title** ("Ubuntu, relational ethics & the Just AI framework"), the **about.html** line ("the RIA Just AI framework"), and check **1.4**. Touches index.html + nav for the 8.3 title — handle as a small separate pass, not silently inside the Part V build.
